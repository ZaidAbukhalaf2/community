...
class Chat{
    ...
    private $user  = 'root';
    private $password   = "";
    private $database  = "orange_family";      
	...
	public function getUserChat($from_user_id, $to_user_id) {
		...
		$conversation = '<ul id="send">';
		foreach($userChat as $chat){
			$user_name = '';
			if($chat["sender_userid"] == $from_user_id) {
				$conversation .= '<li  class="sent">';
				$conversation .= '<img  src="image/'.$fromUserAvatar.'" alt="" />';
			} else {
				$conversation .= '<li class="replies">';
				$conversation .= '<img  src="image/'.$toUserAvatar.'" alt="" style="display: flex;justify-content: flex-end; align-items: flex-end;" />';
			}			
			$conversation .= '<div id="sppan"><span>'.$chat["message"].'</span></div>';			
			$conversation .= '</li>';
		}		
		$conversation .= '</ul>';
		return $conversation;
	...
	public function showUserChat($from_user_id, $to_user_id) {		
		...
		foreach ($userDetails as $user) {
			$toUserAvatar = $user['status'];
			$userSection = '<img src="image/'.$user['img'].'" alt="." />
